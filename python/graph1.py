...
captureRate = 100 # Hz 
holdingFactor = 0.1 # pixel holding factor, range of 0-1 (e.g. if same value is held until next sample point, then holding factor equals to 1)
pxlSize = 0.0078 # unit: degrees; this is the pixel size of a 3840 pixel display with an angle FOV of 30 degrees. 
objSizeNpxl = math.ceil(objSize/pxlSize)# size of the object, in number of pixels
# print(objSizeNpxl)
fillF = 0.5 # pixel fill factor, range of 0-1
antialiasing = True # if use Gaussian antialiasing
antialiasingF = 3 # ignored if antialiasing is False
## ----------- end of user input
...
for i in range(len(t_s)): 
    ...
    for j in range( len(stairCaseRange) ):
        # print("SDSDDSDSfsdklfnsdklfndskfnkdnsfnkjdnkjsfn", stairCaseRange[j], stairCaseRange[0])
